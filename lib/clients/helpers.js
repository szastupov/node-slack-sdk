...
var getData = function getData(data, token) {
  ...
  // NOTE:
  //   - there are a couple of API calls that don't use tokens, so check before passing it through
  //   - also, support users passing in a custom token for an API call
  if (token && !newData.token) {
    newData.token = token;
  }
