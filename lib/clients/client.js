...
function BaseAPIClient(token, opts) {
  ...
  if (this._token) {
    this._requestQueues[this._token] = async.priorityQueue(
      bind(this._callTransport, this),
      clientOpts.maxRequestConcurrency || 5
    );
  }
...
/**
 * Returns the request queue for request retry logic for requests on the supplied token.
 * @param token
 * @returns {*}
 */
BaseAPIClient.prototype.getRequestQueueForToken = function getRequestQueueForToken(token) {
  return this._requestQueues[token];
};
...
BaseAPIClient.prototype.makeAPICall = function makeAPICall(endpoint, optData, optCb) {
  ...
  requestQueue = this.getRequestQueueForToken(apiCallArgs.data.token);
